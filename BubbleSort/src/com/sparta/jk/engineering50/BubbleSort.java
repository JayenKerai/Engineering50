...
public class BubbleSort  <T extends Comparable<T>> {
    private T[] intArray; //contains the array to be sorted

    public BubbleSort(T[] intArray) {
    ...
    private void swap(int i){ //swaps ints at i and i+1
        T temp = intArray[i]; //stores int at i in temp
    ...
    private boolean isBiggerThanNext(int i){ //if i is greater than i+1, return true, else return false
        return (intArray[i].compareTo(intArray[i+1]) > 0 );
    }
